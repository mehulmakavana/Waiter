...
class Menu extends React.Component {
  ...
  async addCart(_id, priority, quantity,email) {
    try {
      const response = await fetch("http://localhost:8020/cart/waiter/addtocart/" + _id, {
        method: "POST",
        body: JSON.stringify({
          priority: priority,
          qty: quantity,
                  ...
                  <div className="cardItem-menus">
                    ...
                    <div className="content-data">
                      ...
                      <div >
                        ...
                        <div className="Order-Note" htmlFor="Order-Name">Email Id</div>
                        <div>
                          <input className="input-notes" type="text" name="email" onChange={(e) => this.handleEmail(e)} />
                        </div>

                      </div>
                      <button className="addCart" onClick={() => this.addCart(person._id, this.state.priority, this.state.quantity,this.state.email)}>Add to Cart</button>
                    </div>
